...
int main(int argc, char* argv[]) {
	...
	SKR_World* world = SKR_CreateWorld(9.81f, 0.2f, SKR_SIDESCROLLER);
	...
	character.w = 50;
	character.h = 150;
	SKR_DynamicRect* chx = SKR_CreateDynamicRect(world, &character, 79.9f, 2.5f, 2.1f);
	...
	while (1) {
		...
		while (SDL_PollEvent(&event)) {
			...
			if (event.key.repeat == 0 && event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_w && SKR_IsOnground(chx)) {
				jump = 1;
			}
		}

		if (jump == 1) {
			SKR_ApplyForceY(chx, -850.5f);
			jump = 0;
		}
		if (right == 1 && left != 1) {
