...
SKR_World* SKR_CreateWorld(float Gravity, float AirFrictionCoefficient, SKR_GAMETYPE GameType);//creates a SKR world to hold static kinematic and dynamic rects' data//you must define your gametype here SKR_SIDESCROLLER or SKR_ISOMETRIC, when you set this SKR_ISOMETRIC gravity will be 0 regardless of your input
...
void SKR_SimulateWorld(SKR_World* World, float Milliseconds);//simulates the world for some time, you must put this in your main game loop. for example your one frame/cycle takes 15 milliseconds, so you should write it like SKR_SimulateWorld(world, 15);